...
Solution GAsolutionDb::GetBestSolution(int argc, char* argv[])
{
	...
	for (int i = 0; i < Params::GA_ITERATIONS; i++)
	{
		...
		{
			//cout << "PROC: " << Params::RANK << " BEST SOLUTION FOUND: " << s.ToString() << endl;
	...
	s = p.Solutions.at(0);
	// cout << "PROC " << Params::RANK << "BEST SOLUTION: " << s.ToString() << endl;
	return s;
}
...
Solution GAsolutionDb::LaunchMasterDb(Population p)
{
	...
	vector<vector<int>> progressReport;
	progressReport.resize(Params::PROCS);
		...
		try
		{
			...
			case ProcRequestRanks:
				...
				if(!SolutionDatabase.empty() && SolutionDatabase.size() > 100)
				{
					...
					MPI_Isend(dbRankOffsets.data(), MASTER_RANK_RESPONSE_SIZE, MPI_INT, procRank, 0, MPI_COMM_WORLD, &request);
				...
				progressReport[procRank].push_back(1);
				
				thisIterIsDone = true;
				d = 0;
				for(vector<int> progr : progressReport)
				{
					if(d != 0 && progr.size() < iterDone)
					{
						thisIterIsDone = false;
					}
					d++;
				}
				if (thisIterIsDone)
				{
					...
					// cout << "This iter best demand: " << bestDemand << endl;
					iterDone++;
				}
				break;
			case ProcFinished:
